...
import type { DrawingPoint, DrawingStroke, CompressedStroke } from '../types/drawing';
...
export const compressStrokeUltraCompact = (
	...
	const relativePoints: [number, number][] = stroke.points.map(
		(p) => [Math.round((p.x - minX) * precision), Math.round((p.y - minY) * precision)] as [number, number]
	);
 ...
 */
export const decompressStrokeUltraCompact = (compressed: UltraCompactCompressedStroke): DrawingStroke => {
...
export const compressStrokeRLE = (stroke: DrawingStroke): RLECompressedStroke | DrawingStroke => {
	...
	// Adjust threshold based on scale to maintain constant visual error
	// Use 0.5 screen pixels as threshold
	const threshold = 0.5;
...
export const compressStrokeVectorQuantization = (stroke: DrawingStroke, gridSize: number = 2): VectorQuantCompressedStroke => {
	...
	const quantized = stroke.points.map((p) => [
		Math.round(p.x / effectiveGrid) * effectiveGrid,
		Math.round(p.y / effectiveGrid) * effectiveGrid,
	]);
 ...
 */
export const decompressStrokeVectorQuantization = (compressed: VectorQuantCompressedStroke): DrawingStroke => {
	return {
