...
import type { DrawingPoint, DrawingStroke, CompressionResult, CompressedStroke } from '@types';
...
const segmentsIntersect = (ax: number, ay: number, bx: number, by: number, cx: number, cy: number, dx: number, dy: number) => {
	...
	if (Math.abs(denom) < 1e-12) {
		...
		const minAx = Math.min(ax, bx),
			maxAx = Math.max(ax, bx);
		const minAy = Math.min(ay, by),
			maxAy = Math.max(ay, by);
		const minCx = Math.min(cx, dx),
			maxCx = Math.max(cx, dx);
		const minCy = Math.min(cy, dy),
			maxCy = Math.max(cy, dy);
...
const segmentIntersectsRect = (ax: number, ay: number, bx: number, by: number, r: { x: number; y: number; w: number; h: number }) => {
...
export const pointToSegmentDistanceSq = (px: number, py: number, ax: number, ay: number, bx: number, by: number): number => {
	const dx = bx - ax;
	const dy = by - ay;
