...
const RedoButton = memo<RedoButtonProps>(({ onRedo, disabled }) => (
  <button
    onClick={onRedo}
    disabled={disabled}
    className={`group cursor-pointer rounded-full p-3 shadow-md transition-all duration-300 text-white ${disabled ? 'bg-neutral-700 opacity-50 cursor-not-allowed' : 'bg-neutral-800 hover:bg-neutral-700'
      }`}
