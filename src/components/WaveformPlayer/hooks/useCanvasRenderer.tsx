...
export const useCanvasRenderer = () => {
	...
	const renderDrawingsOnCanvas = useCallback(
		(ctx: CanvasRenderingContext2D, notesList: Note[]) => {
			...
			const exclude =
				movingStrokePreview && movingStrokePreview.noteId === drawingNote.id
	...
	const renderCanvas = useCallback(() => {
		...
		if (duration > 0 && waveformData && waveformData.length > 0) {
			...
			for (let i = 0; i < waveformData.length; i++) {
				...
				if (!isInTrimRange) {
					fillColor = '#262626'; // Dimmed for trimmed out portions
				} else {
					const relativeProgress = (sampleTime - effectiveTrimStart) / trimmedDuration;
					const isPlayed = currentTime >= effectiveTrimStart && relativeProgress < progress;
					fillColor = isPlayed ? '#a3a3a3' : '#404040';
				}
