	...
	const signIn = useCallback(async (email: string, password: string, fromAnon?: boolean) => {
		...
		try {
			if (prevAnonUid) {
				...
				if (ids.length > 0) {
					...
					const notesData = await fetchProjectNotes(prevAnonUid, projectId);
					const notes = notesData ? notesData.notes : null;
					if (meta) {
		...
		setTimeout(async () => {
			try {
				...
				if (existing) {
					const audio =
						meta.audio && meta.audio.name && meta.audio.size !== undefined && meta.audio.type
